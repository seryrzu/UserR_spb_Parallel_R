  ...
  \begin{table}[]
    ...
    \begin{tabular}{rl}
      ...
      \textit{Problems}: & 
        ...
        \end{minipage} \\
      \pause
      {\color{blue} \textit{Warning:}} & \begin{tabular}{@{}l@{}} \color{blue} Jobs started by \texttt{multicore} share the same state (because of fork) \end{tabular} 
    \end{tabular}
  \end{table}
\end{frame}
