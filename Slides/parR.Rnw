...
\begin{frame}[fragile]{Load Balancing}
<<size = 'tiny', fig.width=8, fig.height=5, out.width='.89\\linewidth', echo=FALSE>>=
plot(tm)
@    
\end{frame}
...
This technique is called ``load balancing''. Function clusterApplyLB() uses that technique.
<<echo=FALSE>>=
library("snow")
cl <- makeCluster(4, type="SOCK")
@
...
sleeptime <- rep(1, 100)
@
  For the first time let's try \texttt{clusterApply()}.
\end{frame}

\begin{frame}[fragile]{Example of comparison clusterApply() and parLapply()}
<<size = 'tiny', fig.width=10, fig.height=5, out.width='.89\\linewidth', echo=FALSE>>=
...
\begin{frame}[fragile]{Example of comparison \texttt{clusterApply()} and \texttt{parLapply()}}
  ...
  Ideally we should have 25 seconds... Let's give \texttt{parApply()} a try.
\end{frame}
...
\begin{frame}
  ...
  \pause
  \textit{General Use case}: provides implicit parallelism, main word is {\color{blue} fork} (thus no Windows support).\\
  \textit{Example}: \texttt{lapply()} runs for ages on your Intel Core i999.\\
  \textit{Solves}: Single-threaded.\\
  \textit{Problems}:
  \begin{itemize}
  ...
  \end{itemize}
  {\color{blue} \textit{Warning:} Jobs started by \texttt{multicore} share the same state (because of fork)}.
\end{frame}
...
\begin{frame}{\texttt{Multicore: mclapply}}
  ...
  \texttt{mclapply(X, FUN, ..., \\
         \quad mc.preschedule = TRUE, mc.set.seed = TRUE,\\
  ...
  \begin{enumerate}
    ...
    \item \texttt{mc.silent = FALSE} --- info from `stdout' for all parallel forked processes? `stderr' is not affected;
...
\begin{frame}{\texttt{Multicore: mclapply, mc.preschedule}}
  \begin{columns}[T]
    ...
    \begin{column}{.48\textwidth}
      Rule of thumb: use
      \begin{itemize}
        \item TRUE when you don't need load balance (for instance, if there are lot's of values in the data);
        \item FALSE when the variance of job completion is very high (so, you need load balance).
      \end{itemize}    
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{\texttt{parallel}}
  \textit{General Use case}: almost a wrapper of \texttt{snow} and \texttt{multicore} packages; main word is {\color{blue} mainstream};\\
  \textit{Example}: anything above;\\
  \textit{Solves}: Single-threaded and (partially) memory-bound;\\
  \textit{Cool features}:
  \begin{itemize}
    \item Preinstalled into \textbf{R} since 2.14.0;
    \item Full RNG support with no dependency on \textit{rlecuyer} package;
  ...
  \end{itemize}
  {\color{blue} \textit{Warning:} On Windows you can't use more than one machine. It also can be difficult to configure multiple Linux machines.}
\end{frame}
...
mc.cores
@
  {\color{blue} \textit{Warning:} It is important to take into account that you can have hyper-threading.}
\end{frame}
...
\begin{frame}{Differences from \texttt{multicore} and \texttt{snow} and new stuff}
...
\begin{frame}{Out of scope}
  ...
  \begin{itemize}
    ...
    \item \textbf{R} and GPUs: \texttt{gputools} etc\ldots
  \end{itemize}
\end{frame}
